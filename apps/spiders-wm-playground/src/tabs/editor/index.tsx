...
async function downloadDirectoryNode(
  ...
  const downloadItems = collectDownloadItems(node, rootPath, filesById, buffers);
  ...
  for (const item of downloadItems) {
    downloadTextFile(item.content, formatFallbackDownloadName(node.name, item.relativePath));
  }
}
...
function formatFallbackDownloadName(directoryName: string, relativePath: string) {
  return relativePath.includes("/")
    ? `${directoryName}__${relativePath.replaceAll("/", "__")}`
    : relativePath;
}
