            ...
            <Panel>
                ...
                <div class="grid gap-1 p-2 text-sm text-terminal-muted">
                    {move || {
                        ...
                        vec![
                            ("workspace".to_string(), snapshot.active_workspace_name().to_string()),
                        ...
                        .map(|(label, value)| {
                            view! {
                                <div class="flex gap-3 justify-between py-1 px-2 border border-terminal-border bg-terminal-bg-panel">
                                    <span>{label}</span>
                                    <span class="text-right text-terminal-fg-strong truncate">{value}</span>
                                </div>
                            }
                        })
                        .collect_view()
                        ...
                        {move || {
                            ...
                            view! {
                                <>
                                    ...
                                    {if has_entries {
                                        view! { <BindingEntries entries=binding_state.entries.clone() /> }.into_any()
                                    } else {
                                        view! { <div class="text-terminal-faint">"no bindings parsed"</div> }.into_any()
                                    }}
                                </>
                            }
                        }}
