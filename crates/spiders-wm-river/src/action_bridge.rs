...
pub enum RiverCommand {
    ...
    AssignFocusedWindowToWorkspace { workspace_id: WorkspaceId },
...
pub fn bridge_action(action: &WmCommand) -> RiverCommand {
    match action {
        WmCommand::Spawn { command } => RiverCommand::Spawn { command: command.clone() },
        WmCommand::Quit => RiverCommand::Unsupported { action: "quit" },
        ...
        WmCommand::ActivateWorkspace { workspace_id } => {
            RiverCommand::ActivateWorkspace { workspace_id: workspace_id.clone() }
        }
        WmCommand::FocusWindow { window_id } => {
            RiverCommand::FocusWindow { window_id: window_id.clone() }
        }
        ...
        WmCommand::FocusDirection { direction } => {
            RiverCommand::FocusDirection { direction: *direction }
        }
        WmCommand::SetFloatingWindowGeometry { window_id, rect } => {
            RiverCommand::SetFloatingWindowGeometry { window_id: window_id.clone(), rect: *rect }
        }
        WmCommand::ViewWorkspace { workspace } => {
            RiverCommand::ActivateWorkspace { workspace_id: workspace.to_string().into() }
        }
        WmCommand::ToggleViewWorkspace { workspace } => {
            RiverCommand::ActivateWorkspace { workspace_id: workspace.to_string().into() }
        }
        WmCommand::AssignWorkspace { output_id, .. } => {
            RiverCommand::FocusOutput { output_id: output_id.clone() }
        }
        WmCommand::FocusMonitorLeft => RiverCommand::Unsupported { action: "focus-monitor-left" },
        WmCommand::FocusMonitorRight => RiverCommand::Unsupported { action: "focus-monitor-right" },
        WmCommand::SendMonitorLeft => RiverCommand::Unsupported { action: "send-monitor-left" },
        WmCommand::SendMonitorRight => RiverCommand::Unsupported { action: "send-monitor-right" },
        ...
        WmCommand::ToggleAssignFocusedWindowToWorkspace { .. } => {
            RiverCommand::Unsupported { action: "toggle-assign-focused-window-to-workspace" }
        }
        WmCommand::SwapDirection { direction } => {
            RiverCommand::MoveDirection { direction: *direction }
        }
        WmCommand::ResizeDirection { .. } => {
            RiverCommand::Unsupported { action: "resize-direction" }
        }
        WmCommand::ResizeTiledDirection { .. } => {
            RiverCommand::Unsupported { action: "resize-tiled-direction" }
        }
        ...
        | WmCommand::SelectWorkspace { .. } => {
            RiverCommand::Unsupported { action: "wm-only-command" }
        }
    }
}
...
mod tests {
    ...
    fn spawn_action_bridges_directly() {
        assert_eq!(
            bridge_action(&WmCommand::Spawn { command: "foot".into() }),
            RiverCommand::Spawn { command: "foot".into() }
        );
    }
    ...
    fn view_workspace_bridges_to_activate_workspace() {
        assert_eq!(
            ...
            RiverCommand::ActivateWorkspace { workspace_id: "3".into() }
        );
    }
    ...
    fn swap_direction_bridges_to_move_direction() {
        assert_eq!(
            bridge_action(&WmCommand::SwapDirection { direction: FocusDirection::Left }),
            RiverCommand::MoveDirection { direction: FocusDirection::Left }
        );
    }
}
