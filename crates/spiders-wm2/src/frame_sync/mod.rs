...
use smithay::{
	...
	wayland::compositor::{BufferAssignment, CompositorHandler, SurfaceAttributes, add_blocker, add_pre_commit_hook, with_states},
	...
	add_pre_commit_hook::<SpidersWm, _>(toplevel.wl_surface(), |state, _dh, surface| {
		let (commit_serial, dmabuf, got_unmapped) = with_states(surface, |states| {
			let dmabuf = {
				...
				let got_unmapped = matches!(guard.pending().buffer.as_ref(), Some(BufferAssignment::Removed));
		...
		let matched = record.frame_sync.install_commit_blockers(
			...
			|state, client| {
				...
				state.client_compositor_state(&client)
...
impl WindowFrameSyncState {
	...
	pub(crate) fn track_pending_layout(
		...
		self.pending_configures.push(
			serial,
			PendingLayout { location, size },
			sync_handle.0,
		);
	}

	pub(crate) fn take_ready_layout(&mut self) -> Option<(Point<i32, Logical>, Size<i32, Logical>)> {
		...
		F: Fn(&mut T, Client) + 'static,
	{
		...
		let live_transaction = matched
			.and_then(|matched| (!matched.transaction.is_completed()).then_some(matched.transaction));
		...
		if let Some((blocker, source)) = dmabuf.and_then(|dmabuf| dmabuf.generate_blocker(Interest::READ).ok())
...
impl FrameSyncState {
	...
	pub(crate) fn push_closing_overlay(
		...
		let (Some(CapturedCloseSnapshot(snapshot)), Some(location), Some(SyncHandle(transaction))) = (snapshot, location, sync_handle) else {
			return None;
		};
