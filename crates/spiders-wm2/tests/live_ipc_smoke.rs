...
#[test]
#[ignore = "requires a live graphical session and intentionally runs the real wm2 binary"]
fn wm2_live_ipc_smoke_uses_test_config() {
    if std::env::var_os("SPIDERS_WM_RUN_LIVE_SMOKE").is_none() {
        eprintln!("skipping live wm2 smoke test; set SPIDERS_WM_RUN_LIVE_SMOKE=1 to enable");
        return;
    }

    if std::env::var_os("DISPLAY").is_none() && std::env::var_os("WAYLAND_DISPLAY").is_none() {
        eprintln!("skipping live wm2 smoke test; no DISPLAY or WAYLAND_DISPLAY is available");
        return;
    }
    ...
    let ipc_socket = temp_root.join("wm2.sock");

    let mut wm2 = spawn_wm2(&workspace_root, &authored_config, &cache_dir, &ipc_socket);
...
fn spawn_wm2(
    ...
    Command::new(env!("CARGO_BIN_EXE_spiders-wm2"))
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("failed to spawn wm2 live smoke process")
}
