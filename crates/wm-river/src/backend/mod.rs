...
impl RiverBackendState {
    ...
    fn apply_window_rules(&mut self, window_id: &spiders_core::WindowId) {
    ...
    fn next_output_id(&mut self) -> spiders_core::OutputId {
        self.next_output_serial += 1;
        format!("river-output-{}", self.next_output_serial).into()
    }

    fn next_window_id(&mut self) -> spiders_core::WindowId {
    ...
    fn initialize_new_windows(&mut self) -> Vec<spiders_core::WindowId> {
    ...
    fn start_workspace_transition(&mut self, workspace_id: &spiders_core::WorkspaceId) {
...
mod tests {
    ...
    fn configured_mode_rule_prefers_fullscreen_over_floating() {
        ...
        assert_eq!(mode, Some(spiders_core::types::WindowMode::Fullscreen));
    }
