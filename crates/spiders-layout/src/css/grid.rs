...
impl<'i> DeclarationParser<'i> for GridFallbackDeclarationParser {
    ...
    type Error = super::syntax::CssParseError;
    ...
    fn parse_value<'t>(
        ...
        if !super::syntax::SUPPORTED_PROPERTIES.contains(&property.as_str()) {
            return Err(input
                .new_custom_error(super::syntax::CssParseError::UnsupportedProperty { property }));
        }
        let value_start = input.state();
        let components = super::syntax::parse_component_values(input)?;
        ...
        compile_declaration(&parsed)
            .map_err(|error| input.new_custom_error(super::syntax::CssParseError::CssValue(error)))
    }
}
...
impl<'i> RuleBodyItemParser<'i, CompiledDeclaration, super::syntax::CssParseError>
...
pub(super) fn parse_grid_fallback_declarations(
    input: &str,
) -> Result<Vec<CompiledDeclaration>, super::syntax::CssParseError> {
        ...
        match item {
            ...
            Err((err, _)) => {
                return Err(match err.kind {
                    ...
                    _ => super::syntax::CssParseError::InvalidSyntax {
