    ...
    R: AuthoringLayoutRuntime<Config = Config>,
{
    ...
    let events = match action {
        ...
        WmAction::ViewWorkspace { workspace } => {
            let output_id = wm_state.current_output_id()?.clone();
            recompute = true;
            wm_state.view_workspace_on_output(&output_id, *workspace)?
        }
        WmAction::ToggleViewWorkspace { workspace } => {
            let events = wm_state.toggle_workspace_on_current_output(*workspace)?;
        ...
        WmAction::AssignFocusedWindowToWorkspace { workspace } => {
            let event = wm_state.assign_focused_window_to_workspace(*workspace)?;
            recompute = true;
            vec![event]
        }
        WmAction::ToggleAssignFocusedWindowToWorkspace { workspace } => {
            let event = wm_state.toggle_assign_focused_window_to_workspace(*workspace)?;
            recompute = true;
            vec![event]
        }
...
mod tests {
    ...
    #[test]
    fn toggle_view_workspace_switches_when_workspace_is_not_currently_visible() {
        let mut runtime = runtime_state();
        let mut wm_state = WmState::from_snapshot(state());
        ...
        let outcome = apply_action(
            &mut runtime,
            &mut wm_state,
            &WmAction::ToggleViewWorkspace { workspace: 2 },
        )
        .unwrap();
        ...
        assert!(
            outcome
                .events
                .iter()
                .any(|event| matches!(event, CompositorEvent::WorkspaceChange { .. }))
        );
    }

    #[test]
    fn toggle_view_workspace_is_noop_for_currently_visible_workspace() {
        let mut runtime = runtime_state();
        let mut wm_state = WmState::from_snapshot(state());
        ...
        let outcome = apply_action(
            &mut runtime,
            &mut wm_state,
            &WmAction::ToggleViewWorkspace { workspace: 1 },
        )
        .unwrap();
