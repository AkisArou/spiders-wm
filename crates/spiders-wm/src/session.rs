use crate::domain_session::{DomainSession, DomainSessionError, DomainUpdate};
...
fn map_domain_error(error: DomainSessionError) -> ActionError {
    match error {
        DomainSessionError::Wm(error) => ActionError::WmState(error),
        DomainSessionError::Topology(error) => map_topology_error(error),
    }
}
...
mod tests {
    ...
    #[test]
    fn session_applies_workspace_switch_action_and_updates_snapshot() {
        let mut session = session();

        let update = session
            .apply_action(&WmAction::ToggleViewWorkspace { workspace: 2 })
            .unwrap();
